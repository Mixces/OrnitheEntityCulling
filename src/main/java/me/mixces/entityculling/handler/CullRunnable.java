	...
	public void run() {
		while (Minecraft.getInstance() != null && ((MinecraftAccessor) Minecraft.getInstance()).getRunning()) {
			try {
				...
				if (world == null || minecraft.getCamera() == null) continue;
				List<Entity> entities = new CopyOnWriteArrayList<>(minecraft.world.getEntities());
				CullHandler.ENTITY_CULL_RESULTS.keySet().removeIf(networkId -> world.getEntity(networkId).getUuid() == null);
				...
				for (Entity entity : entities) {
					...
					CullHandler.ENTITY_CULL_RESULTS.put(entity.getNetworkId(), shouldCull);
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
